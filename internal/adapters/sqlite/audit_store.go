...
func (as *auditStore) ListByTicket(ctx context.Context, ticketID int64) ([]domain.AuditEvent, error) {
	rows, err := as.db.QueryContext(ctx,
		`SELECT ticket_id, actor, action, field, from_value, to_value, note, created_at
	...
	for rows.Next() {
		...
		var field, fromValue, toValue, note sql.NullString
		var createdAt string
		if err := rows.Scan(&e.TicketID, &e.Actor, &e.Action, &field, &fromValue, &toValue, &note, &createdAt); err != nil {
