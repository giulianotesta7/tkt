...
func TestMigrateCreatesSchema(t *testing.T) {
	...
	if applied != 6 {
		t.Errorf("schema_migrations rows = %d, want 6 (through 0006_category_workflows)", applied)
	}
	...
	if len(versions) != 6 || versions[0] != 1 || versions[1] != 2 || versions[2] != 3 || versions[3] != 4 || versions[4] != 5 || versions[5] != 6 {
		t.Errorf("versions = %v, want [1 2 3 4 5 6]", versions)
	}
}
...
func TestMigrateRerunIsNoOp(t *testing.T) {
	...
	if applied != 6 {
		t.Errorf("rerun recorded %d versions, want 6 (no-op)", applied)
	}
}
