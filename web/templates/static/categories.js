(() => {
  ...
  const drawerState = (panel) => ({[KEY]: 1, kind: "drawer", closeURL: panel.dataset.closeUrl || listURL(), launcherKey: openerKey || ""});
  ...
  function openDrawer() {
    ...
    if (!same(state, {[KEY]: 1})) {
      if (opener || (state && location.pathname.includes("/edit"))) {
        history.replaceState(drawerState(panel), "", url);
      } else {
        history.replaceState(baseState(close), "", close);
        history.pushState(drawerState(panel), "", url);
      }
  ...
  document.addEventListener("click", (event) => {
    ...
    if (event.target.closest(".category-drawer-close,.category-drawer-cancel,.category-drawer-backdrop")) {
