(() => {
  ...
  const controls = () => {
    ...
    if (!form) return null;
    return {
      source: form.elements.namedItem("source_index"),
      target: form.elements.namedItem("target_index"),
      button: form.querySelector("[data-workflow-reorder]"),
    };
  };
  const terminalSlots = () =>
    cards().filter((card) => !card.querySelector(".workflow-drag-handle")).length;
  let source = -1;
  let destination = -1;
  let indicator = null;
  ...
  const moveIndicator = () => {
    ...
    if (destination < 0 || destination >= list.length) indicator.style.display = "none";
      ...
      indicator.style.left = list[destination].offsetLeft - 2 + "px";
    }
  };
  const clearDrag = () => {
  ...
  document.addEventListener("dragstart", (event) => {
    const handle =
      event.target instanceof Element ? event.target.closest(".workflow-drag-handle") : null;
  ...
  const focusValidationControl = () => {
    ...
    builder
      .querySelector(
        ".workflow-editor-panel textarea, .workflow-editor-panel input:not([type='hidden']), .workflow-editor-panel select",
      )
  ...
  const positionDropdown = (details) => {
    const body = details.querySelector(
      ".workflow-step-menu-actions, .workflow-add-options, .workflow-field-menu-actions",
    );
    if (!body) return;
    if (!details.open) {
      body.style.position = "";
...
const trackable = (el) =>
  !!el.name && el.type !== "submit" && !el.disabled && !UNTRACKED_FIELDS.has(el.name);
...
document.addEventListener("htmx:beforeRequest", (event) => {
  ...
  pendingStructural = {
    ...
    path: event.detail?.pathInfo?.requestPath ?? builderForm()?.action ?? "",
...
document.addEventListener("click", (event) => {
  ...
  const choices = [
    ["save", "[data-workflow-save-continue]"],
    ["discard", "[data-workflow-discard-continue]"],
    ["cancel", "[data-workflow-cancel]"],
  ];
