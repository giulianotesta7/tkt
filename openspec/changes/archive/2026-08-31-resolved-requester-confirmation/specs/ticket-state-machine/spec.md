...
Scope note: this is the spec-phase delta for change `resolved-requester-confirmation` (issue #55). It redefines `resolved` as awaiting requester confirmation, makes `resolved` → `closed` conditional on three closure paths, and adds the requester rejection path that detaches the workflow. No canonical file, runtime, template, test, or migration is edited here. The audit attribution mechanism behind closure-path recording is a design decision and is not specified here.
