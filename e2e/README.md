...
All screens below are asserted via `e2e/tests/helpers/layout.ts` (`collectObservability` + `assertCanonicalScreen`) at **390px** and **1280px** for both **empty-base** (bootstrap) and **seeded** profiles where indicated. The helper captures `console.error`, `pageerror`, failed loopback requests, and loopback 5xx responses; failures report screen label, URL, and role. External origins are ignored (app is loopback-only).
...
| Users | `/users` | 390, 1280 | root | h1 Users, New user link, overflow + in-panel scroll at 390px, observability | Users — list | `tests/users.spec.ts`, `tests/structural.spec.ts` | — | `handlers_users_view_test.go` |
...
| Tickets — search filter real swap | search `q` via `hx-get` → `#ticket-list`; assert region innerHTML changes, filtered result visible, empty state on no-match, header intact, complementary `hx-get`/`hx-target` | region content changed AND header/URL intact | `tests/tickets.spec.ts`, `tests/htmx.spec.ts` | exhaustive combos → Go |
| Tickets — public comment | fill comment body → POST → timeline contains comment → reload persists | timeline content change | `tests/tickets.spec.ts`, `tests/ticket-detail.spec.ts` | exhaustive visibility → Go |
| Tickets — transition | select `#ticket-state` → POST /transition → #ticket-detail re-render + state badge In Progress, timeline entry, reload persists | region content changed, state visible | `tests/tickets.spec.ts`, `tests/ticket-detail.spec.ts` | all state edges → Go state-machine tests |
...
| Categories — workflow integrated | create category → open workflow → add Manual task (assert count +1 + live region, URL unchanged, hx-target complementary) → remove (assert count -1) → publish (POST 200, badge Published) → reload persistence → create ticket using category → verify category linkage; publish version visibility limitation documented (stamp not surfaced in browser without product changes) | builder innerHTML changed, publish 200, ticket creation succeeds with new category | `tests/categories.spec.ts` | exhaustive workflow validations → Go |
...
| HTMX — partial swaps | ticket list filter, users tabs (`#users-root`), workflow builder (`#workflow-builder`), ticket detail (`#ticket-detail`) — each asserts target region content changed AND surrounding chrome intact AND URL (unchanged or only hx-push-url) | region innerHTML not equal before/after + header intact + complementary hx-* | `tests/htmx.spec.ts` | — |
