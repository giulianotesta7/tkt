...
| Categories/workflows — integrated | create category → open workflow → add Manual task (count+1, live region, `assertHtmxSwap` on `/categories/{id}/workflow`) → edit Instructions with zero autosave POSTs → explicit Save (live `Saved`) → remove (count-1) → re-add → edit Instructions → publish (POST 200, live `Published`) → reload persistence → create ticket with category → `#workflow-pending` + `.workflow-instruction` show `Handle the ticket` on ticket detail | `tests/categories.spec.ts` |
